...
export function generateButton(key: string, className: string, clickFunctionName: string, label?: string, setDisabledCondition = "") {
...
export function generateFieldHTML(propertyWithPath: PropertyWithPath, themeObject: any, name: string) {
    ...
    const textNode = doc.createTextNode(`{{ ${strings.camelize(name)}?.${propertyWithPath.htmlPath} }}`);
...
export function generateTable(themeObject: any, name: string, properties = new Array<PropertyWithPath>(), idFieldName = 'id') {
    ...
    properties.forEach((element, index) => {
        ...
        tableHeading.innerHTML = (!isLast) ? element.label : themeObject.headerActions.label;
    ...
    tableRow.setAttribute(AngularAttribute.NG_FOR_LOOP, `let ${itemName} of ${pluralName}; let i = index;`);
    ...
    properties.forEach((element, index) => {
        ...
        if (isFirst) {
            ...
            anchorTag.setAttribute(AngularAttribute.NG_ROUTER_LINK, `['/${itemName}', ${itemName}?.${idFieldName}]`);
            ...
            editLink.setAttribute(AngularAttribute.NG_ROUTER_LINK, `['/${itemName}/edit', ${itemName}?.${idFieldName}]`);
            ...
            const deleteButton = generateButton(deleteAction.label, deleteAction.class, `${deleteAction.clickFunctionName}(${itemName})`);
    ...
    if (counter > 0) {
        ...
        const editButton = generateButton("Edit", themeObject.actionButtonClass, "onEditClick({item,rowIndex:i})");
        const deleteButton = generateButton("Delete", themeObject.actionButtonClass, "onDeleteClick(i)");

        createCellWithTemplates(tableRow, [editButton, deleteButton]);
    }

    const noRow = doc.createElement('tr');
    noRow.setAttribute(AngularAttribute.NG_IF, `!(${pluralName} amp;amp; ${pluralName}?.length)`);

    const combineCell = doc.createElement('td');
    combineCell.setAttribute(AngularAttribute.NG_COLSPAN, (properties.length).toString());

    const cellText = doc.createTextNode(`{{ '${themeObject.emptyTableMessage}' }}`);
