...
export function getEachPropertyWithPath(schemaDefinition: any, keyValuePair = new Array<any>(), previousPath = '', previousHTMLPath = '') {
    ...
    Object.entries(schemaDefinition)
        .map((props) => {
            ...
            if (property.type === "group") {
                const _key = !isNullOrUndefined(previousPath) ? `${previousPath}.${key}` : key;
                const htmlPath = !isNullOrUndefined(previousPath) ? `${previousPath}?.${key}` : key
                getEachPropertyWithPath(property.property, keyValuePair, _key, htmlPath);
            } else if (property.type === "array") {

            } else {

                keyValuePair.push(new PropertyWithPath(
                    key,
                    strings.capitalize(key),
                    !isNullOrUndefined(previousPath) ? `${previousPath}.${key}` : key,
                    !isNullOrUndefined(previousHTMLPath) ? `${previousHTMLPath}?.${key}` : key
                ));
            }
        });
    return keyValuePair;
