...
export function updateRouteFile(options: any, workspace: any): Rule {
  return (tree: Tree, _context: SchematicContext): Tree => {
    ...
    const text =
      importsObject.importDeclarationText &&
    ...
    if (routerInstance.some(e => text.includes(e))) {
      ...
      const cleanedText = text
        .slice(routerModuleIndex, routerModuleEndIndex + 1)
        .replace(/\s/g, '');
      cleanedText.endsWith('[])');

      // const routeObject = findRouteConst(moduleFile);

      const routeUpdate = (routeObject.isEmptyArray ? '' : ' , ').concat(
        `${getRoutes(options)}`
      );
      ...
      const importUpdate = (importsObject.isEmptyArray ? '' : ' , ').concat(
  ...
  return `
    {
        path : '${strings.dasherize(
          options.name
        )}', component : ${strings.classify(
    options.name + '-' + ComponentType.LIST
  )}Component
    },
    {
        path : '${strings.dasherize(
          options.name
        )}/create', component : ${strings.classify(
    options.name + '-' + ComponentType.EDIT
  )}Component, pathMatch:'full'
    },
    {
        path : '${strings.dasherize(
          options.name
        )}/:id', component : ${strings.classify(
    options.name + '-' + ComponentType.DETAILS
  )}Component
    },
    {
        path : '${strings.dasherize(
          options.name
        )}/edit/:id', component : ${strings.classify(
    options.name + '-' + ComponentType.EDIT
  )}Component
    }`;
};
